...
class NutshellCRM(BaseCRM):
    def __init__(self, **credentials):
        super().__init__(**credentials)
        self.base_api_url = self.base_url.rstrip('/') if self.base_url else "https://api.nutshell.com/v1"
               ...
               res = client.get(f"{self.base_api_url}/leads", headers=self.headers)
                if res.status_code == 200:
                    for item in res.json().get('data', []):
                        if item.get('phone'):
                            leads.append({"external_id": item['id'], "first_name": item.get('first_name',''), "last_name": item.get('last_name',''), "phone": item['phone'], "source": "Nutshell"})
